  ...
  function exportStatSheet() {
    const attackTypeReport = attackTypes
      .map((type) => {
        ...
        return `${type}: ${typeAttacks.length} attacks | ${typeSuccessful} successful | ${percent(
          typeSuccessful,
          typeAttacks.length
        )}% efficiency | Avg phases ${average(typeAttacks.map((a) => a.phases || 0))}`;
      })
      .join("\n");

    const report = `${matchName || "MATCH ANALYSIS"}${opposition ? ` vs ${opposition}` : ""}
...
GOLD ZONE
Entries: ${goldZoneEntries.length}
Successful Entries: ${successfulGoldZoneEntries.length}
Gold Zone Efficiency: ${percent(successfulGoldZoneEntries.length, goldZoneEntries.length)}%
Points Generated: ${goldZonePoints}

KICKING
Contestable Kicks: ${contestableKicks.length}
Kick Regained: ${kickRegained}
Kick Lost: ${kickLost}
Contestable Kick Effectiveness: ${percent(kickRegained, contestableKicks.length)}%

Exit Kicks: ${exitKicks.length}
Good Exits: ${goodExits}
Bad Exits: ${badExits}
Exit Success: ${percent(goodExits, exitKicks.length)}%

SET PIECE
Lineouts: ${lineoutsWon}W / ${lineoutsLost}L | Success: ${percent(lineoutsWon, totalLineouts)}%
Scrums: ${scrumsWon}W / ${scrumsLost}L | Success: ${percent(scrumsWon, totalScrums)}%
...
`;
    ...
    link.href = url;
    link.download = `${matchName || "rugby-analysis"}-stat-sheet.txt`;
    link.click();
  ...
  function exportPDFReport() {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(18);
    doc.text("RUGBY ANALYSIS REPORT", margin, 14);
    ...
    doc.save(`${matchName || "rugby-analysis"}-coach-report.pdf`);
  }
  ...
  if (activeTool === "clips" || activeTool === "moves") {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0B1120] p-8 text-white">
        <div className="max-w-2xl rounded-3xl border border-slate-800 bg-slate-950 p-10 text-center shadow-2xl">
          ...
          <h1 className="mb-4 text-5xl font-black">
            {activeTool === "clips" ? "Auto Clip Creator" : "Moves Designer"}
          </h1>
          <p className="mb-8 text-lg text-slate-300">
            {activeTool === "clips"
              ? "This will become the tool for generating compilation videos from tagged match events."
      ...
      <div className="sticky top-0 z-40 border-b border-slate-800 bg-[#020617]/95 px-4 py-3 shadow-2xl backdrop-blur">
        <div className="grid grid-cols-3 items-center gap-3">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <button
              onClick={exportStatSheet}
              className="rounded-lg bg-cyan-400 px-4 py-2 text-sm font-bold text-slate-950 shadow-lg shadow-cyan-400/20 transition hover:scale-105 hover:bg-cyan-300 active:scale-95"
            >
              Export Stat Sheet
            </button>
      ...
      <div className="p-4">
        <section className="mb-4 rounded-2xl border border-slate-800 bg-slate-950 p-3 shadow-xl transition hover:border-slate-700">
          <div className="mb-3 flex items-center justify-between">
            ...
            <p className="text-sm text-slate-400">V3.4.4 Analyst Workflow</p>
          </div>
